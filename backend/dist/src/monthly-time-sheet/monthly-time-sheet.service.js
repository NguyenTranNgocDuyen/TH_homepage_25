...
let MonthlyTimeSheetService = class MonthlyTimeSheetService {
    ...
    async getMonthlyTimesheetsForReview(month, year, currentManagerId) {
        ...
        if (month !== expectedPeriod.month || year !== expectedPeriod.year) {
            throw new common_1.BadRequestException(`Managers can only review submitted timesheets for the previous month (${expectedPeriod.month}/${expectedPeriod.year}).`);
        }
        const timesheets = await this.prismaService.monthlyTimesheet.findMany({
            where: {
                month,
    ...
    async SubmitMonthlyTimesheet(monthlyTimesheetID, tx) {
        try {
            const executeLogic = async (dCbt) => {
                ...
                const notificationGet = await this.notificationService.createNotification(userGet.data.userID, department.data.managerID, `Bảng công tháng ${monthGet.month}/${monthGet.year} của ${userGet.data.username} đang chờ duyệt.`, client_1.NotificationRelatedType.TIMESHEET, dCbt);
    ...
    async reviewMonthlyTimesheet(monthlyTimesheetID, reviewMonthlyTimesheetDto, reviewerID, tx) {
        ...
        try {
            const executeLogic = async (dCbt) => {
                ...
                const expectedPeriod = getPreviousMonthPeriod();
                if (monthGet.month !== expectedPeriod.month ||
                    monthGet.year !== expectedPeriod.year) {
                    throw new common_1.BadRequestException(`Only previous month timesheets (${expectedPeriod.month}/${expectedPeriod.year}) can be reviewed.`);
                }
                if (accept &&
                    monthGet.entries.some((entry) => !entry.checkOut || entry.status === client_1.TimesheetStatus.MISSING_OUT)) {
                    throw new common_1.BadRequestException('Cannot approve a monthly timesheet with missing check-out entries.');
                }
                const now = new Date();
                ...
                const notificationGet = await this.notificationService.createNotification(reviewerID ?? department.data.managerID, userGet.data.userID, `Bảng công tháng ${monthlyTimesheetUpdate.month}/${monthlyTimesheetUpdate.year} của bạn ${reviewStatus}.${rejectReason}`, client_1.NotificationRelatedType.TIMESHEET, dCbt);
                if (notificationGet.statusCode !== code_1.CREATED_RESPONE)
                    throw new common_1.BadRequestException(notificationGet.message);
                try {
                    await this.emailService.sendTimesheetNotification({
