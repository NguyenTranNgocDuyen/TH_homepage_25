...
let RequestCorrectionService = class RequestCorrectionService {
    ...
    async createRequest(userID, dto) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            const notification = await this.notificationService.createNotification(userID, managerID, `${timesheet.employee.username} requested a timesheet correction for ${timesheet.month}/${timesheet.year}.`, client_1.NotificationRelatedType.TIMESHEET, tx);
    ...
    async reviewRequest(requestCorrectionID, reviewerID, dto) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            const notification = await this.notificationService.createNotification(reviewerID, request.userID, `Your timesheet correction request was ${dto.status}.${dto.status === client_1.TimesheetStatus.REJECTED ? ` Reason: ${reasonReject}` : ''}`, client_1.NotificationRelatedType.TIMESHEET, tx);
            if (notification.statusCode !== code_1.CREATED_RESPONE) {
                return {
                    statusCode: notification.statusCode,
                    message: notification.message,
