    ...
    async checkIn(userID, IPAddress, deviceInfo, tx) {
        ...
        try {
            const executeLogic = async (dbCtx) => {
                ...
                return { statusCode: code_1.CREATED_RESPONE, message: 'Check-in successful!' };
            };
            if (tx)
        ...
        try {
            ...
            if (result.statusCode === code_1.OK_CODE && isWarning && managerIdToNotify) {
                this.notificationService
                    .createNotification('system', managerIdToNotify, `Cảnh báo: Nhân viên ${userName} Check-out với IP khác (${IPAddress}) so với lúc Check-in.`, client_1.NotificationRelatedType.WARNING)
